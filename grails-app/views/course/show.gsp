	...
	<body>
		...
		<div id="show-course" class="content scaffold-show" role="main">
			...
			<ol class="property-list course">
				...
				<g:if test="${courseInstance?.studentList}">
				<li class="fieldcontain">
					<span id="studentList-label" class="property-label"><g:message code="course.studentList.label" default="Student List" /></span>
					
						<g:each in="${courseInstance.studentList}" var="s">
						<span class="property-value" aria-labelledby="studentList-label"><g:link controller="student" action="show" id="${s.id}">${s?.encodeAsHTML()}</g:link></span>
						</g:each>
					
				</li>
				</g:if>
			
				<g:if test="${courseInstance?.subjectList}">
				<li class="fieldcontain">
					<span id="subjectList-label" class="property-label"><g:message code="course.subjectList.label" default="Subject List" /></span>
					
						<g:each in="${courseInstance.subjectList}" var="s">
						<span class="property-value" aria-labelledby="subjectList-label"><g:link controller="subject" action="show" id="${s.id}">${s?.encodeAsHTML()}</g:link></span>
						</g:each>
